function visWeeklyScore(queryEng, varargin)


%fin_sherbert for Fin Whale Scores
%grabs detections based on call -- 20Hz it will grab 1440minute bins,
% 40Hz it will use 60 minute bins
...
dbSpeciesFmt('Input','Abbrev','SIO.SWAL.v1');%temp
...
if ~(req_input==5)
    error('Missing an Argument: Project, Deployment, Site, SpeciesID, Call');
end
...
plots(num_plots) = struct('Site','','Deployments',[],'SpeciesID','',...
    'Call','','Subtype','','Effort',[],'Bins',[],'days_of_data',...
    ...
    for cidx=1:length(info.spp(spidx).Calls)
        for sidx=1:site_count
            ...
            for didx=1:depl_count
                ...
                if ~isempty(queries(quer_count).Subtype)
                ...
                if iseff
                    if subbed
                        ...
                        [queries(quer_count).Detections,jnk,scores] = ...
                            ...
                            queries(quer_count).Deployment,...
                            'SpeciesID',queries(quer_count).SpeciesID,'Call',...
                            queries(quer_count).Call,'Return',...
                            'Parameters/Score');
                    end
                    ...
                    if isempty(queries(quer_count).Detections)
                        ...
                        endtime_pholder = zeros(length(starts_vec),1);
                        %populate Bins matrix

                        %store detection scores next to bins
                        scorevec = str2double(scores.Score);
% 
%                         if firstdets
%                             plots(plotIdx).Bins(:,3) = scorevec;
%                             firstdets = 0;
%                         else
%                             plots(plotIdx).Bins(:,3) = [plots(plotIdx).Bins(:,3); scorevec];
%                         end
                        plots(plotIdx).Bins = [plots(plotIdx).Bins; datenum(starts_vec),endtime_pholder, scorevec];
                    ...
                    queries(quer_count).DetCount =...
                        length(queries(quer_count).Detections); %perhaps good to know?
...
for pidx = 1:length(plots)
    %set end time
    for ridx = 1:length(plots(pidx).Bins)
        if bsize == 60
            plots(pidx).Bins(ridx,2) = addtodate(...
                plots(pidx).Bins(ridx,1),60,'minute');
        elseif bsize==1440
            %disp(ridx);
            plots(pidx).Bins(ridx,2) = addtodate(...
                plots(pidx).Bins(ridx,1),1440,'minute');
        end
    end
    
    %find NaN scores and remove associated bins
    score_indices=find(isnan(plots(pidx).Bins(:,3)));
    if ~isempty(score_indices)
        score_indices=flipud(score_indices);%start bot
        for scridx=score_indices
            plots(pidx).Bins(scridx,:)=[];
        end
    end
end
...
if isempty(info.effIdx)
    disp('No Effort  found for this inquiry');
    return;
end
...
for i=1:length(plots)
    ...
    %%%LOOP THRU EFFORTS, TRUNCATE START/END IF SHORT BREAKS%%%
...
%   perdayscaled,       a cell array with days and their scores
...
for pidx=1:length(plots); %plot index for queries struct
    ...
    startvec = datevec(xstart);
    endvec = datevec(xstop);
    effstartvec = datevec(plots(pidx).Effort(1,1));
    startvec(1) = effstartvec(1);
    startvec(3) = 1; % pad to first of the month
    effendvec = datevec(plots(pidx).Effort(end,2));
    endvec(1) = effendvec(1);
    ...
    spdatediff = bsxfun(@minus,(plots(pidx).Bins(:,2)), (plots(pidx).Bins(:,1)));
    ...
    % Get mean of scores that happened on same day...
    %
    starttimes=floor(plots(pidx).Bins(:,1));
    k2 =1;
    idx = 1;
    dayscores = [];
    while idx<=length(starttimes)
        ...
        dayscores(k2,1)= starttimes(idx);
        dayscores(k2,2)= mean(plots(pidx).Bins(same_days,3));
        k2=k2+1;
        idx=max(same_days)+1;
    end
    ...
    info.perdayscaled{pidx}=[dayscores(:,1), dayscores(:,2)]; % populate
    ...
    for i = 1:days_with_hits
        ...
        if size(index,1) == 1
            ...
            disp(['Inconsistent data. Multiple or 0 entries for a day, pidx=',num2str(pidx),', i=',num2str(i)]);
            return
        end
    end
    ...
    if ~isempty(sharing) % we only want to do this on spp with data
        for pidx = sharing
            ...
            for i = 1:num_of_weeks
                ...
                plots(pidx).mean_score(i,1) = mean(data(first_day:last_day,2));
                plots(pidx).pcnt_eff(i,1) = (sum(data(first_day:last_day,3)))/(7*24)*100;
            end
            
            sp_hrs_all = [sp_hrs_all; plots(pidx).mean_score];
...
for n=1:length(plots);
    ...
    label = '';
    fun1 = @(x,y)plot(x,y,'-s','Color',[0 0 0]);
    fun2 = @(x,y)plot(x,y,'k.','MarkerSize',20,'Color',[0.5 0.5 0.5]);
    t = [];
    
    t = plots(n).week(:,1);
    y = plots(n).mean_score(:,1);
    e = plots(n).pcnt_eff(:,1);
    t2 = t; e2 = e;
    
    for yidx = 1:length(y)
        %replace scores of 0 with NaN so those points dont get plotted
        if y(yidx)==0
            y(yidx) = NaN;
        end
        yidx=yidx+1;
    ...
    ylabel(AX(1),'Average Score per Week','FontSize',12);
