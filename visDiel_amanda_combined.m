function visDiel_amanda_combined(queryEng, varargin)
...
xtick = 1; %default 3
ytick = 14; %default 7
resolution=1; %hourly resolution.  default 60
        ...
        for sidx=1:site_count
            ...
            for didx=1:depl_count
                ...
                if subbed && ~grouped
                    ...
                    if clicks
                        z=[];
                        z = dbGetEffort(queryEng,'Project',project,'Site',...
                            queries(quer_count).Site,'Deployment',...
                            queries(quer_count).Deployment,'SpeciesID',...
                            queries(quer_count).SpeciesID,'Call',...
                            queries(quer_count).Call,'Subtype',...
                        ...
                        if isempty(info.efforts{quer_count})
                            info.efforts{quer_count} = z;
                        else
                            info.efforts{quer_count} = cat(1,info.efforts{quer_count},z);
                            info.efforts{quer_count} = sortrows(info.efforts{quer_count});
                        end
                    elseif whistles
                        z=[];
                        z = dbGetEffort(queryEng,'Project',project,'Site',...
                ...
                if iseff
                    ...
                    if subbed && ~grouped
                        ...
                        if clicks
                            s=[];
                            s = dbGetDetections(queryEng,'Project',project,'Site',...
                                queries(quer_count).Site,'Deployment',...
                                queries(quer_count).Deployment,...
                                'SpeciesID',queries(quer_count).SpeciesID,'Call',...
                                queries(quer_count).Call,'Subtype',...
                            ...
                            if isempty(queries(quer_count).Detections)
                                queries(quer_count).Detections=s;
                            else
                                queries(quer_count).Detections = cat(1,queries(quer_count).Detections,s);
                                queries(quer_count).Detections = sortrows(queries(quer_count).Detections);
                            end
                        elseif whistles
                            s=[];
                            s = dbGetDetections(queryEng,'Project',project,'Site',...
                    ...
                    if isempty(queries(quer_count).Detections)
                        ...
                        elseif strcmp(granularity,'call')
                            ...
                            if size(plots(plotIdx).Detections,2) >1 
                                plots(plotIdx).Detections(:,2) = [];
                            end
                        else
                            plots(plotIdx).Detections = [plots(plotIdx).Detections; queries(quer_count).Detections];
                        end
...
pstart = datenum([2013 05 13 00 00 00]); %plot start date JAX    
pstop = datenum([2014 09 01 00 00 00]);  %plot stop date JAX 
if start
    pstart = usr_start;
end
if stop
...
for n=1:length(plots)
    ...
    if grouped
        ...
        title = sprintf('Plot %d: %s%s%s - %s - %s.%s', ...
            n, project,deployments, site, species, call,'Combined');
    end
    rect = [0, 0, 500, 800];
